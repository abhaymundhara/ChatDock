...
const PLAN_FILE = path.join(process.env.HOME || '', '.chatdock', 'current_plan.json');
...
const todo_write = {
  name: 'todo_write',
  description: 'Creates or updates a task list/plan for the current request. Helps organize complex multi-step tasks.',
  parameters: {
    type: 'object',
    properties: {
      title: {
...
const ask_user = {
  name: 'ask_user',
  description: 'Pauses execution to ask the user a clarifying question. Use when you need more information to proceed.',
  parameters: {
    type: 'object',
    properties: {
      question: {
...
const think = {
  name: 'think',
  description: 'Triggers extended thinking mode for complex reasoning. Use for multi-step analysis, debugging, or when you need to reason through a problem carefully.',
  parameters: {
    type: 'object',
    properties: {
      problem: {
...
const summarize_context = {
  ...
  description: 'Creates a summary of the current conversation context to save tokens.',
  parameters: {
    type: 'object',
    properties: {
      keepRecent: {
  ...
  run: async ({ keepRecent = 5 }) => {
    ...
    return {
      ...
      keepRecent,
      instruction: 'Summarize the earlier conversation and keep only the last N messages in full.'
    };
  }
};
